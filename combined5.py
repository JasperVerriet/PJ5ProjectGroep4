...
def Timetable_comparison(df):
    ...
    dfc['start time'] = pd.to_datetime(dfc['start time'],format='%H:%M:%S',errors='coerce')
    ...
    for bus_id, bus_routes in group_bus:
        ...
        for route_index, route in bus_routes.iterrows():
            ...
            if current_battery_level - energy_consumption < min_battery_level:
                print(f"\nBus {bus_id}: Battery level will drop below 10% during route {route_index+1}. Route is infeasible.")
                feasible = False
                break
